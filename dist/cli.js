...
Promise.all([codePromise, argv.theme ? fs.readFile(argv.theme, 'utf8') : undefined])
    ...
    var options = {
        ignoreIllegals: true,
        theme: (theme && (0, theme_1.parse)(theme)) || undefined,
    };
    if (file) {
        var extension = path.extname(file).slice(1);
        if (extension && (0, _1.supportsLanguage)(extension)) {
            options.language = extension;
        }
    }
    options.language = argv.language;
    return new Promise(function (resolve, reject) {
        return process.stdout.write((0, _1.highlight)(code || '', options), function (error) { return (error ? reject(error) : resolve()); });
    });
})
    .then(function () {
    process.exit(0);
